...
namespace FreeDNS_Client
{
    /// <summary>
    /// Handles a log file
	/// http://dotnet-snippets.de/snippet/logwriter-klasse/746
	/// Veröffentlicht von v.wochnik am 30.12.2007
    /// </summary>
    ...
    public class LogWriter : Component
    {
        ...
        /// </summary>
        private string messageFormat = "{0:} {0:t}: {1}";
        #endregion
        ...
        public bool WriteMessage(string message)
        {
            ...
            try
            {
                if (weekly)
                {
                    fileName += $"\\week_{GetIso8601WeekOfYear(time).ToString("00")}.log";
                }
                else
            ...
            {
                // creates the line
                line = $"{time.ToString("MM-dd HH:mm")}  {message}";//String.Format(this.messageFormat, new object[] { time, message });
            }
            catch (FormatException)
            {
